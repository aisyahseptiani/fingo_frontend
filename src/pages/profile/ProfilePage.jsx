    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-gray-900">Profil Akun</h1>
            <p className="text-gray-400 text-sm">Informasi, pencapaian & kesehatan keuanganmu</p>
          </div>
        </div>
        <button
          onClick={logout}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        {/* Aktivitas keuangan */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <h2 className="font-bold text-gray-900 mb-1">Aktivitas Keuangan</h2>
          <p className="text-xs text-gray-400 uppercase tracking-widest font-semibold mb-4">Breakdown Skor 87</p>
          <div className="space-y-4">
            {ACTIVITY_SCORES.map((a, i) => (
              <div key={i}>
                <div className="flex justify-between text-sm mb-1.5">
                  <span className="text-gray-600">• {a.label}</span>
                  <span className={`font-bold ${a.color === 'bg-[#22c55e]' ? 'text-[#22c55e]' : a.color === 'bg-yellow-400' ? 'text-yellow-500' : 'text-blue-500'}`}>
                    {a.percent}%
                  </span>
                </div>
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div className={`h-2 rounded-full ${a.color} transition-all`} style={{ width: `${a.percent}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-gray-900">Target Keuangan</h2>
            <button className="flex items-center gap-1.5 px-3 py-1.5 bg-[#22c55e]/10 border border-[#22c55e] text-[#22c55e] text-xs font-semibold rounded-xl hover:bg-[#22c55e]/20 transition-colors">
            ...
            {TARGETS.map((t, i) => (
              <div key={i}>
                <div className="flex justify-between items-start mb-1">
                  ...
                  <span className={`text-sm font-bold ${t.textColor}`}>{t.percent}%</span>
                </div>
                <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden mb-1">
                  <div className={`h-1.5 rounded-full ${t.color}`} style={{ width: `${t.percent}%` }} />
                </div>
                <div className="flex justify-between text-xs text-gray-400">
                  <span>{formatRp(t.current)} dari {formatRp(t.goal)}</span>
                  <span className={t.est.includes('🔥') ? 'text-orange-500 font-semibold' : ''}>{t.est}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-5 p-4 bg-blue-50 rounded-xl border border-blue-100">
            <p className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-1">Saran AI untuk Target</p>
