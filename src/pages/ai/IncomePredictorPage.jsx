...
const HISTORY_TABLE = [
  { period: 'Mg ini (Aktual)', amount: 1050000, source: 'Gojek + Freelance', vsPred: 7,   highlight: true },
  { period: 'Minggu lalu',     amount: 980000,  source: 'Gojek',             vsPred: 2    },
  { period: '2 Minggu lalu',   amount: 1120000, source: 'Freelance Design',  vsPred: -3   },
  { period: '3 Minggu lalu',   amount: 850000,  source: 'Gojek',             vsPred: -13  },
  { period: '4 Minggu lalu',   amount: 1030000, source: 'Gojek + Bonus',     vsPred: 5    },
]

const INCOME_SOURCES = [
  'Gojek', 'Grab', 'Freelance Design',
  'Shopee', 'Gojek + Freelance', 'Lainnya',
]
...
export default function IncomePredictorPage() {
  ...
  const splitIndex = data.findIndex(d => d.pred !== undefined) - 1
  const splitLabel = data[splitIndex]?.label ?? 'Mg 1'
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">
            Visualisasi historis dan periksa pendapatan 4 minggu ke depan
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Prediksi bulan ini',   value: 'Rp 4.4 jt',  sub: '↑Lebih baik dari biasanya',  valueColor: 'text-blue-500'   },
          { label: 'Akurasi model',        value: '87%',         sub: 'Berdasarkan 12 minggu data', valueColor: 'text-[#22c55e]'  },
        ].map((s, i) => (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        {/* Legend manual — tidak pakai Legend dari recharts */}
        <div className="flex justify-end gap-5 mb-3">
          <div className="flex items-center gap-2">
            <div className="w-6 h-0.5 bg-[#22c55e]" />
            <span className="text-xs text-gray-500 font-medium">Historis</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 border-t-2 border-dashed border-yellow-400" />
            <span className="text-xs text-gray-500 font-medium">Prediksi AI</span>
          </div>
        </div>
          ...
          >
            ...
            <Line
              ...
              dot={(props) => {
                ...
                const isLast = data[index]?.label === splitLabel
                return (
                  <circle
                    key={`dot-h-${index}`}
                    cx={cx}
      ...
      {/* Bawah: Tabel + Panel kanan */}
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-5">

        {/* Tabel rincian historis */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            <h2 className="font-bold text-gray-900">Rincian Data Historis</h2>
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/60">
                <th className="text-left px-6 py-3 text-xs font-bold text-gray-400 uppercase tracking-wider">Periode</th>
                <th className="text-left px-3 py-3 text-xs font-bold text-gray-400 uppercase tracking-wider">Pendapatan</th>
                <th className="text-left px-3 py-3 text-xs font-bold text-gray-400 uppercase tracking-wider">Sumber Utama</th>
                <th className="text-right px-6 py-3 text-xs font-bold text-gray-400 uppercase tracking-wider">VS Prediksi</th>
              </tr>
            </thead>
              ...
              {HISTORY_TABLE.map((row, i) => (
                <tr key={i} className="hover:bg-gray-50/60 transition-colors">
                  <td className="px-6 py-3.5">
                    <span className={`font-semibold text-sm ${row.highlight ? 'text-[#22c55e]' : 'text-gray-800'}`}>
                      {row.period}
                    </span>
                  </td>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="space-y-2">
              {[
                { label: 'Minggu depan (+1)', value: 1100000 },
                { label: '+2 minggu',          value: 1150000 },
                { label: '+3 minggu',          value: 1200000 },
                { label: '+4 minggu',          value: 1250000 },
              ].map((p, i) => (
                <div key={i} className="flex items-center justify-between py-1.5">
                  <span className="text-sm text-gray-600">{p.label}</span>
                  <span className="text-sm font-black text-yellow-500">
                    Rp {(p.value / 1000000).toFixed(2).replace('.', ',')} jt
                  </span>
                </div>
              ))}
            </div>
            <div className="border-t border-gray-100 mt-3 pt-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500">Total prediksi 4 minggu :</span>
                <span className="text-sm font-black text-gray-900">Rp 4.7 jt</span>
              </div>
            </div>
          </div>
